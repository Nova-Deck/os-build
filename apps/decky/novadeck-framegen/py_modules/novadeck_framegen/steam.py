...
DUPLICATED FROM novadeck-control ON PURPOSE, not extracted into a shared module. Decky loads
each plugin's py_modules in its own process, and a module staged in by the image build would be
invisible to the offline suite, which imports these straight from the repo checkout. The same
reasoning keeps _clean_env duplicated across the other two plugins. Keep the two copies in step
by hand; tests/test-decky.sh checks that they agree.
