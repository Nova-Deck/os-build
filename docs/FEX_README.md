...
so no new guest has to be built. `rootfs/lib-assemble-storage.sh` surfaces it as **two** fstab rows: the
...
checked by reading committed files. `rootfs/lib-assemble-storage.sh` reads it out of the image at build
