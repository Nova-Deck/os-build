...
/usr/share/decky-plugins/novadeck-control  baked plugin (lib-assemble-decky-splash 4e)
...
- Adding a first-party plugin touches three lists that must stay in step: `DECKY_PLUGINS` in the
  Makefile, the `for plugin_name in …` loop in `rootfs/lib-assemble-decky-splash.sh` (4e), and assertion
