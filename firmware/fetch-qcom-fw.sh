...
# Idempotent — a staged tree that already verifies against its sha256sums.txt is left
# untouched; FORCE=1 re-fetches.
set -euo pipefail
...
# Idempotent: skip if the staged tree already verifies against its sha sidecar.
if [ -z "${FORCE:-}" ] && [ -f "$DEST/sha256sums.txt" ] \
