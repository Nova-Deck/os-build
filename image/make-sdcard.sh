...
# (ota/publish-card.sh) — ~44% of every download — bought to cover a failover window that closes
...
# lays the GPT with sgdisk via image/genpart.sh, and dd's each filesystem into its partition
...
# rootfs/build-image.sh (-> out/images/{rootfs,var}.img) have run.
set -euo pipefail
...
if [ "$SLOT_B" = 1 ]; then
  [ -f "$VARIMG_B" ] || { echo "no var-b image: ${VARIMG_B#"$ROOT"/} (run rootfs/build-image.sh)" >&2; exit 1; }
...
#
# THE LAYOUT ITSELF LIVES IN image/lib-homestage.sh, because the internal installer builds the same
...
fixed_mib=$("$ROOT/image/genpart.sh" --min)
...
# Slot A needs no copy at all: rootfs/assemble-rootfs.sh already labels rootfs.img novadeck-root-A,
...
"$ROOT/image/genpart.sh" "$IMG" >/dev/null
sgdisk -p "$IMG"
...
loader:
partitions:
comment: seeded by image/make-sdcard.sh
EOF
}
