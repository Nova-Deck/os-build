...
#
# Same doctrine as rootfs/guard-rootfs.sh: assert the BUILT ARTIFACT, not the source diff. That
...
#   docker run --rm -v "$PWD":/src -w /src novadeck-build image/verify-card.sh
set -uo pipefail
...
# kernel (image/make-sdcard.sh documents the mechanism). A B.conf on a card with an empty B is a
