...
| `partition-table.txt` | The 9-partition A/B layout (ESP, A/B boot, A/B root, A/B /var, shared seed, shared /home). Single source of truth for sizes, typecodes and labels. |
...
| `initramfs/init` | The initramfs `/init`: mounts the root read-only, mounts `/var`, stacks the `/etc` overlay on it, mounts the shared seed, then `switch_root`s into systemd. Degrades to a writable un-overlaid root (loudly, via `/dev/kmsg`) rather than bricking a device with no serial console. |
...
| `assemble-rootfs.sh <base-rootfs>` | Stage base + kernel + firmware, then split the tree into the three images the table wants: `rootfs.img` (btrfs, ro), `var.img` (ext4, carries the `/etc` overlay upper), `seed.img` (squashfs, shared recovery). All unprivileged. Also injects InputPlumber config for all boards, the `session/` gamescope-session overlay, and the offload bind units. |
| `make-sdcard.sh` | Lay the **full** GPT from the table and populate only the A side (ESP + root-A + var-A + seed + home) → `out/images/sdcard.img`. The B slots and both `efi-*` partitions are created but empty, so adding RAUC later never needs a reflash. Unprivileged (mtools + sgdisk + `dd`, no loop). |
