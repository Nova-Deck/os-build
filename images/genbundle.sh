...
#
#   RAUC_CERT=cert.pem RAUC_KEY=key.pem images/genbundle.sh <soc> [version]
#
...
if [ -z "$CERT" ] || [ -z "$KEY" ]; then
  ...
  openssl req -x509 -newkey rsa:2048 -nodes -keyout "$KEY" -out "$CERT" \
    -days 7 -subj "/O=novadeck/CN=novadeck-dev-${SOC}" >/dev/null 2>&1
fi

# Bundle content dir: manifest (placeholders filled) + the rootfs image.
content="$work/content"; mkdir -p "$content"
sed -e "s/@SOC@/$SOC/g" -e "s/@VERSION@/$VERSION/g" "$TEMPLATE" >"$content/manifest.raucm"
cp "$ROOTFS" "$content/rootfs.img"

rm -f "$BUNDLE"
echo "[novadeck] bundling $SOC v$VERSION -> ${BUNDLE#"$ROOT"/}"
