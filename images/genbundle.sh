...
#
#   RAUC_CERT=cert.pem RAUC_KEY=key.pem images/genbundle.sh
#
...
# IMAGE's version (/etc/novadeck-release, stamped by images/assemble-rootfs.sh from
...
[ -f "$RELEASE" ]  || {
  ...
  echo "(images/assemble-rootfs.sh writes it beside rootfs.img; re-run it — rm $ROOTFS && make rootfs)" >&2
  exit 1
}
