...
#
#   images/seal-rootfs.sh <staged-root>
#
# Runs on the STAGED tree inside images/assemble-rootfs.sh, not on work/base. That placement is
# deliberate: work/base stays a faithful record of what was installed, so images/genmanifest.sh
...
#
# WHAT IT REMOVES is declared in images/seal.list (committed, reviewed) — read that file first;
...
STAGE="${1:?usage: images/seal-rootfs.sh <staged-root>}"
LIST="$ROOT/images/seal.list"
...
echo "  sealing: stripping ${#pkgs[@]} packages + ${#paths[@]} paths (images/seal.list)"
