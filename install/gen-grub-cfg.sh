...
#
#   install/gen-grub-cfg.sh <root-partuuid> <outfile>
#
...
#
# THE MENU IS THE BOARD PICKER, exactly as on the shipped card. One image serves every board and
# the DTB is the one thing no autodetection here can settle, so the user picks once and the choice
# is saved to grubenv on this same ESP. There is no keyboard on these devices — the d-pad drives
# the menu — so the menu stays VISIBLE even after a choice is saved. Picking wrong is otherwise
# unrecoverable without a PC.
#
...
[ -n "$PARTUUID" ] && [ -n "$OUTFILE" ] \
  || { echo "usage: install/gen-grub-cfg.sh <root-partuuid> <outfile>" >&2; exit 2; }
...
#                         differently (three writers pointed at /run — see install/mkroot.sh);
...
: >"$OUTFILE"
cat >>"$OUTFILE" <<EOF
# novadeck INSTALLER stage-2 GRUB configuration — GENERATED by install/gen-grub-cfg.sh.
# Do not edit on the medium; rebuild the image.

# The saved board choice. Saved on this ESP, which is the only writable filesystem GRUB can reach
# here. savedefault is NOT a GRUB builtin — grub-mkconfig emits it as a shell function in the
# configs it generates, and a config that CALLS it without DEFINING it prints "unknown command"
# per menuentry and silently saves nothing.
load_env -f \$prefix/grubenv saved_entry
...
if [ -n "\${saved_entry}" ]; then
  ...
  # Nothing can tell us the board, so wait indefinitely. -1 is "no timeout".
  set default=0
  set timeout_style=menu
  set timeout=-1
fi
