#!/usr/bin/env bash
# novadeck INSTALLER manifest generator -> installer/manifest.lock  (Phase 6).
#
#   installer/genlock.sh [installer-rootfs-dir]        (default: work/installer-base)
#
...
#     installer keeps its pacman, deliberately (see installer/pkgs.list).
#   - it refuses a `dev:1` base. installer/mkroot.sh has no dev mode to refuse.
#   - it walks packages/*/prebuilt.pin by auto-discovery, which is exactly the behaviour
#     installer/pygame-ce.pin exists OUTSIDE packages/ to avoid. This one reads the prebuilt rows out
#     of the marker installer/mkroot.sh wrote into the tree, so the lock describes the tree that was
...
#   prebuilt  not pacman packages at all: the sha256-pinned tarballs/wheels installer/mkroot.sh
...
[ -d "$LOCALDB" ] || {
  ...
  echo "  NOVADECK_RESOLVE=1 installer/mkroot.sh" >&2
  exit 1
}
...
if [ "$missing" -gt 0 ]; then
  ...
  echo "  cleared since the root was built -> rebuild: FORCE=1 NOVADECK_RESOLVE=1 installer/mkroot.sh" >&2
  exit 1
fi
...
{
  cat <<EOF
# novadeck INSTALLER package manifest — GENERATED by installer/genlock.sh, do not hand-edit.
# Regenerate with \`make relock-installer\` after any change to installer/pkgs.list; review the diff.
#
# This describes the INSTALLER image (installer/mkroot.sh), NOT the image that ships on a device —
