#!/usr/bin/env bash
# Phase 4 hardware STAGER for installer/novadeck-install — the destructive peer of
# installer/hw-select-target.sh. Plan verification step 3.
#
#   installer/hw-install.sh [options] root@<device>
#
...
#
#   1. FOUR BINARIES ARE ABSENT — sgdisk, mdir, mkfs.vfat, partprobe. See installer/lib-hwstage.sh,
...
if [ -f "$SEED_SRC" ]; then
  ...
  # build/steam-seed/fetch-steam-seed.sh makes, and the same escape hatch applies: delete the tarball to
  ...
  die "no Steam seed at $SEED_SRC (run build/steam-seed/fetch-steam-seed.sh, or pass --seed)"
fi
...
scp "${SSHOPTS[@]}" -q \
  "$ROOT/installer/novadeck-install" \
  "$ROOT/installer/select-target.sh" \
  "$ROOT/installer/carve.sh" \
  "$ROOT/installer/rauc-session.sh" \
  "$ROOT/installer/post-install-fresh.sh" \
  "$ROOT/installer/confirm-tty" \
...
#!/usr/bin/env bash
# GENERATED by installer/hw-install.sh -- /run is tmpfs, so this is gone at the next reboot and the
...
terminal (installer/confirm-tty), so it needs a tty: -t is not optional.
