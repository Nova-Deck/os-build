...
#
#   installer/hw-preflight.sh root@<device>
#
# Renders the screen installer/ui would draw before the consent gate, against the device's own disk.
...
# lib-gpt.sh and lib-slotwrite.sh beside itself, and installer/ui finds its sibling modules by
# dirname(__file__). The same rule installer/hw-install.sh follows, for the same reason — a card
...
scp "${SSHOPTS[@]}" -q \
  ...
  "$ROOT/installer/release-info" \
  "$ROOT/installer/ui" \
  "$ROOT/installer/uipad.py" \
  "$ROOT/installer/uiflow.py" \
