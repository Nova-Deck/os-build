...
#
#   installer/mkroot.sh                    # locked: installer/manifest.lock, sha256-verified
#   NOVADECK_RESOLVE=1 installer/mkroot.sh # re-resolve from installer/pkgs.list (this is how the lock
...
# no audio, no FEX. See installer/pkgs.list, which is the declaration this reads.
#
...
# installer/mkimage.sh does it instead, for the same reason rootfs/assemble-rootfs.sh rather than
...
# keyboard, and installer/save-log.sh only writes the ESP once systemd has got far enough to STOP the
...
# The FAT label installer/mkimage.sh gives the medium's own ESP, and the /etc/fstab entry below finds
...
# ---- the declaration ---------------------------------------------------------------------------
# installer/pkgs.list is the human-editable intent; the lock is the resolved closure. Comments and
...
# installer/pygame-ce.pin exists OUTSIDE packages/ to stay clear of -- 39 MB of Python SDL bindings
# that only installer/ui uses would otherwise be pure weight on every shipped device. So this image
...
#   pygame-ce     the SDL binding installer/ui draws with. There is no python-pygame and no PySDL2 in
...
PINS=("$ROOT/packages/inputplumber/prebuilt.pin" "$ROOT/installer/pygame-ce.pin")
...
# aggregates them into its install list automatically; here they are LEAVES of installer/pkgs.list,
...
# The from-source overlay repo is REQUIRED, not optional: installer/pkgs.list names gamescope, which
...
# explicitly rather than globbed: installer/ also holds the hw-* stagers (build-host only), the test
...
# ...and three the spine resolves by SEARCH, which live outside installer/ because the shipped image
...
for f in "${INSTALL_FILES[@]}"; do
  [ -f "$ROOT/installer/$f" ] || die "installer/$f is missing — the installer cannot ship without it"
done
...
# installer/pkgs.list has no `linux-firmware`. The shipped image gets firmware from these two staging
...
# `compgen -G` trap in rootfs/guard-rootfs.sh and the `grep -q` one in installer/netcfg: a status from
...
# installer/mkimage.sh stages it and derives the pin from the bytes it placed -- because this tree is
...
# One row per pin, sorted: `name version sha256 kind dest strip`. installer/genlock.sh reads the
...
# Every file that lands in the tree, hashed. These are the installer -- an edit to installer/ui or to
...
payload=""
for f in "${INSTALL_FILES[@]}"; do payload+="$(sha256sum "$ROOT/installer/$f")"$'\n'; done
...
for f in "$ROOT"/installer/units/*.service "$DEVICE_ENV" "$DEVICES_DIR"/*.conf "$KEYRING_SRC"; do
...
for f in "${INSTALL_FILES[@]}"; do cp -p "$ROOT/installer/$f" "$STAGE/flat/$f"; done
for spec in "${FOREIGN_FILES[@]}"; do cp -p "$ROOT/${spec%%:*}" "$STAGE/flat/${spec##*:}"; done
cp -p "$ROOT"/installer/units/*.service "$STAGE/units/"
...
docker run --rm --platform linux/arm64 \
  ...
  # (which Includes the mirrorlist the host wrote from build/snapshot.pin), so the vendor /etc/pacman.conf
  ...
  # which is where it belongs -- installer/genlock.sh reads that database to write the lock.
  ...
  # ---- units --------------------------------------------------------------------------------------
  # Named, not globbed: installer/units/ also holds novadeck-installer-hostkey.service, which is a
  ...
  # 2. NetworkManager profiles. installer/netcfg creates a connection with nmcli, and the keyfile
  ...
  printf "%s\n" \
    "# novadeck installer: the root is read-only squashfs (installer/mkroot.sh)." \
    "[keyfile]" \
  ...
  # Network: NetworkManager owns the links (installer/netcfg drives nmcli), so mask networkd. Both
  ...
  if [ -f /prebuilt/authorized_keys ]; then
    ...
    # The key generator is a COMMITTED unit file (installer/units/), not text printf-ed from here.
  ...
  # and installer/genlock.sh refuses to lock a tree without it.
...
#   the rootfs path, which the Makefile and installer/mkimage.sh capture.
