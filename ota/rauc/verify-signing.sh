...
#   - the bundle format genbundle.sh produces from ota/rauc/manifest.raucm.in is one the shipped
...
#   - that ota/rauc/novadeck-ca.pem is the CA behind ota/rauc/release.cert.pem. Both are
...
#   docker run --rm -v "$PWD":/src -w /src novadeck-build ota/rauc/verify-signing.sh
set -euo pipefail
...
# — it mints both ends — so none of it can notice that ota/rauc/novadeck-ca.pem came from a CA
