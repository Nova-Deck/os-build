#!/usr/bin/env bash
# novadeck lock materializer — turn rootfs/manifest.lock into a verified install list
# (Phase 4a step 2). Host-side: no container, no emulation, everything it needs is a file.
#
#   rootfs/fetchlock.sh <install-list-out> [lock]     (default lock: rootfs/manifest.lock)
#
# Writes one CONTAINER-visible package path per line to <install-list-out>, which
# rootfs/customize-base.sh stages into the bootstrap container and feeds to a single
...
#             rootfs/seal-rootfs.sh deletes them from the release tree afterwards (Phase 4a
