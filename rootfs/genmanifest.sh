...
# path, which is an alias tracking the newest revision (see build/snapshot.pin). Revisions can
...
snapshot="$(grep -vE '^[[:space:]]*(#|$)' "$ROOT/build/snapshot.pin" 2>/dev/null | tail -1 || true)"
builder="$(grep -vE '^[[:space:]]*(#|$)' "$ROOT/build/base-devel.digest" | tail -1)"
