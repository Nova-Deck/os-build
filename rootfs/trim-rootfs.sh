...
#
#   rootfs/trim-rootfs.sh <staged-root>
#
# The size counterpart to rootfs/seal-rootfs.sh, and deliberately a separate step: the seal is
# about what a sealed root can DO (it removes the package manager), this is about what it WEIGHS.
# Runs on the STAGED tree inside rootfs/assemble-rootfs.sh, after the seal, so work/base keeps
# being a faithful record of what was installed and `make relock` is unaffected.
#
# WHAT IT REMOVES is declared in rootfs/conf/trim.list (committed, reviewed) — read that file first;
...
# states what it no longer has — and rootfs/guard-rootfs.sh re-expands that copy against the
...
STAGE="${1:?usage: rootfs/trim-rootfs.sh <staged-root>}"
LIST="$ROOT/rootfs/conf/trim.list"
...
echo "  trimming: ${#paths[@]} paths + ${#globs[@]} globs (rootfs/conf/trim.list)"
